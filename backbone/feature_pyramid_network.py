...
# sys.path.append(os.path.dirname(os.path.abspath("./pvt.py")))
...
class BackboneWithFPN(nn.Module):
    ...
    def __init__(self,
        ...
        self.afpn = AFPN.AFPN(in_channels=[256, 512, 1024, 2048], out_channels=256)
        self.pvt = pvt.pvt_medium()
        self.fpn_pvt = FeaturePyramidNetwork(
            in_channels_list=[64, 128, 320, 512, 512],
    ...
    def forward(self, x):
        x_pvt = self.pvt(x)
        x_pvt = self.fpn_pvt(x_pvt)
        x = self.body(x)
        out0, out1, out2, out3 = self.afpn([x['0'], x['1'], x['2'], x['3']])
        ...
        for x1, x_pvt1 in zip(x, x_pvt):
            feature[x1] = torch.cat((x[x1], x_pvt[x_pvt1]), dim=1)
        return feature
