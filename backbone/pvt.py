...
class PyramidVisionTransformer(nn.Module):
    def __init__(self, img_size=384, patch_size=32, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        self.patch_embed5 = PatchEmbed(img_size=img_size // 32, patch_size=2, in_chans=embed_dims[3],
                                       embed_dim=embed_dims[3])
        # pos_embed
        ...
        self.pos_embed44 = nn.Parameter(torch.zeros(1, self.patch_embed4.num_patches, embed_dims[3]))
        self.pos_drop4 = nn.Dropout(p=drop_rate)
        self.pos_embed5 = nn.Parameter(torch.zeros(1, self.patch_embed5.num_patches, embed_dims[3]))
        self.pos_drop5 = nn.Dropout(p=drop_rate)
        # transformer encoder
        ...
        # classification head
        self.head1 = nn.Linear(embed_dims[0], 1)
        self.head2 = nn.Linear(embed_dims[1], 1)
        self.head3 = nn.Linear(embed_dims[2], 1)
        self.head4 = nn.Linear(embed_dims[3], 1)
        self.head5 = nn.Linear(embed_dims[3], 1)
        # init weights
    ...
    def forward_features(self, x):
        ...
        for i in range(x.shape[1]):
            if i==0:
                ...
                x_1=torch.cat((x_1,self.head1(x[:,i,:])),dim=1)
#         x_1=x_1.reshape(B,H,W,-1).permute(0, 3, 1, 2).contiguous()
        x1 = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()#96*96*64
        ...
        for i in range(x.shape[1]):
            if i==0:
                ...
                x_2=torch.cat((x_2,self.head2(x[:,i,:])),dim=1)
#         x_2=x_2.reshape(B,H,W,-1).permute(0, 3, 1, 2).contiguous()
        x2 = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()#48*48*128
        ...
        for i in range(x.shape[1]):
            if i==0:
                ...
                x_3=torch.cat((x_3,self.head3(x[:,i,:])),dim=1)
#         x_3=x_3.reshape(B,H,W,-1).permute(0, 3, 1, 2).contiguous()
        x3 = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()#24*24*320

        # stage 4
        x, (H, W) = self.patch_embed4(x3)
        x = x + self.pos_embed44
        x = self.pos_drop4(x)
        ...
        for i in range(x.shape[1]):
            if i==0:
                ...
                x_4=torch.cat((x_4,self.head4(x[:,i,:])),dim=1)
#         x_4=x_4.reshape(B,H,W,-1).permute(0, 3, 1, 2).contiguous()
        x4 = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()#12*12*512
        
#         # stage 4
#         x, (H, W) = self.patch_embed5(x4)
#         x = x + self.pos_embed5
#         x = self.pos_drop5(x)
...
#                 x_5=self.head5(x[:,i,:])
#             else:
#                 x_5=torch.cat((x_5,self.head5(x[:,i,:])),dim=1)
# #         x_5=x_5.reshape(B,H,W,-1).permute(0, 3, 1, 2).contiguous()
#         x5 = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()#6*6*512
...
def pvt_small(pretrained=False, **kwargs):
    model = PyramidVisionTransformer(
        patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 3, 6, 3], sr_ratios=[8, 4, 2, 1], **kwargs)
    model.default_cfg = _cfg()
