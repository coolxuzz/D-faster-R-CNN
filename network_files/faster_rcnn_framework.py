...
class FasterRCNNBase(nn.Module):
    ...
    def forward(self, images, targets=None):
        ...
        features_res = self.backbone(images.tensors)  # 将图像输入backbone得到特征图
        #         print(features_res.shape)
        # if isinstance(features_res, torch.Tensor):  # 若只在一层特征层上预测，将feature放入有序字典中，并编号为‘0’
        #     features_res = OrderedDict([('0', features_res)])  # 若在多层特征层上预测，传入的就是一个有序字典

        # 将特征层以及标注target信息传入rpn中
        # proposals: List[Tensor], Tensor_shape: [num_proposals, 4],
        # 每个proposals是绝对坐标，且为(x1, y1, x2, y2)格式
        proposals, proposal_losses = self.rpn(images, features_res, targets)

        # 将rpn生成的数据以及标注target信息传入fast rcnn后半部分
        detections, detector_losses = self.roi_heads(features_res, proposals, images.image_sizes, targets)
...
class TwoMLPHead(nn.Module):
    ...
    def __init__(self, in_channels, representation_size):
        ...
        self.fc6 = nn.Linear(in_channels*2, representation_size)
...
class FasterRCNN(FasterRCNNBase):
    ...
    def __init__(self, backbone, num_classes=None,
                 # transform parameter
                 min_size=384, max_size=384,  # 预处理resize时限制的最小尺寸与最大尺寸
